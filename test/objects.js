...
//
  QUnit.test('omit', function(assert) {
    ...
    console.log('result symbol1:   ' + result[symbol1]);
    console.log('foo symbol2:   ' + foo[symbol2]);
    console.log('result symbol2:   ' + result[symbol2]);
    assert.equal(result[symbol1], 1, 'can recognise symbols');
    assert.deepEqual(result, {symbol1: 1, symbol2: 2}, 'can recognise multiple symbols');
    assert.deepEqual(result, {symbol2: 2}, 'can recognise protype symbols');
    // result = _.omit(foo, '[symbol1]','symbol2');
    // assert.deepEqual(result, {a: 0}, 'can omit symbols');
  });
//
