(function() {
  ...
  QUnit.test('unique', function(assert) {
    ...
    assert.deepEqual(_.unique(list), [1, 2, 3, 4], 'can find the unique values of an unsorted array');
    list = [1, 1, 1, 2, 2, 3];
    ...
    var result = (function(){ return _.unique(arguments); }(1, 2, 1, 3, 1, 4));
    assert.deepEqual(result, [1, 2, 3, 4], 'works on an arguments object');
  ...
  QUnit.test('uniq', function(assert) {
    assert.strictEqual(_.uniq, _.unique, 'is an alias for unique');
  });
